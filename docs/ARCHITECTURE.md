...
3. Else, if the machine has **rebooted** since the job started → the process cannot exist and
   nothing is wrong with the case. Requeue it (`RUNNING → QUEUED`) with `resume_requested = 1`.
4. Else → the process is gone and left no sentinel on a machine that has been up the whole time.
   State `UNKNOWN`, `exit_reason = 'lost'`.
...
**A reboot is not a daemon restart**, and step 2 runs first so a surviving simulation is never
restarted merely because the boot record looks stale. The two are told apart by the kernel's
`btime`, recorded on the job when it starts (`jobs.boot_time`) and compared against the machine's
`btime` now. Boot against boot, from one source: the tempting comparison — "did the machine boot
after the job started?" — quietly assumes the job's timestamps and the system clock share an epoch.
A job with no recorded boot keeps the pre-existing `lost` outcome, because restarting work on no
evidence is worse than admitting ignorance.

**Where a requeued job resumes from is not Dispatch's decision.** The job keeps its `seq`, so it
keeps its exact place in the queue and a reboot cannot reorder the work that was waiting behind it.
When the scheduler next reaches it, the executor builds its context with `resume=True` and asks the
adapter — `resume_point(ctx)` for the audit trail, `plan()` for the steps. The adapter reads the
answer off the disk. Dispatch stores a boolean and never a timestep: it knows a job *was running*,
which is not the same as knowing what the solver *finished writing*, and a number invented here
would eventually resume a run from a state that was never saved. The flag is cleared once the
context carrying it exists, so a restart cannot ask for another one.
...
A directory whose subdirectories are all cases of the same solver can be submitted once, as a
**sweep**: N ordinary jobs plus one extra scheduling rule.

**Detection is conservative**, because the two mistakes do not cost the same. Missing a sweep leaves
the user submitting cases one at a time, which is what they did before; inventing one queues a pile
of jobs nobody asked for. `AdapterRegistry.detect_sweep` requires all of: the directory is not
itself a case; at least two subdirectories are recognised cases; they all detect as the *same*
adapter; and every visible subdirectory is one of them — a single `scripts/` or `results/` sibling
means this is a working directory that happens to contain cases. Each child is identified with
`best_detection`, so a directory two adapters both claim counts as unrecognised rather than being
assigned a solver on a coin toss. Cases are ordered with a natural sort, so `case_10` follows
`case_9` rather than `case_1`.

**A sweep is not a job.** It never runs, holds no cores, has no exit code, and never appears in the
queue; it is a row in `sweeps` plus two nullable columns on its members. Modelling it as one large
job would reserve `cores_per_job × concurrency` as a block — both a lie to the ledger and the
opposite of the point, since the cores a sweep is *not* using must stay available to other work.

**The scheduler change is one filter**, applied where `--after` already is — after dependencies,
before the policy:

```
normal job:  existing resource rules AND existing dependency rules
sweep job:   existing resource rules AND existing dependency rules AND a free slot in its sweep
```

Filtering *before* the policy is what keeps the effect local: the policy is only ever offered jobs
that may actually start, so a policy seeing spare capacity cannot undo the cap, and every other job
is offered exactly the capacity it would have been offered anyway. Jobs with `sweep_id IS NULL` are
returned untouched, which is every job that existed before this feature.

**The limit is a hard cap on running members, never a core budget.** On a 12-core machine, a sweep
of 3 cores/job with concurrency 2 runs two jobs and leaves six cores idle — and an unrelated 6-core
job may take them. Slots are counted as a *union of ids* across three sets, not a sum: members
active in the database, members admitted on an earlier pass whose transition has not landed yet
(`launch` returns before the executor writes it), and members admitted earlier in the same pass.
Adding counts would double-count a job as its transition lands; the first pass after a submission
would otherwise start the whole sweep at once.
